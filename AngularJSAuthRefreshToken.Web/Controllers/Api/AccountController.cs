...
namespace AngularJSAuthRefreshToken.Web.Controllers.Api
{
    ...
    public class AccountController : ApiController
    {
        ...
        public async Task<IHttpActionResult> AddExternalLogin([IPrincipal] IPrincipal principal, AddExternalLoginBindingModel model)
        {
            ...
            {
                return BadRequest(Resources.Common.ExternalLoginAlreadyAssociated);//"The external login is already associated with an account."
            }

            var login = new UserLoginInfo(externalData.LoginProvider, externalData.ProviderKey);

            var user = await UserManager.FindAsync(login);
            if (null != user)
            {
                return BadRequest(Resources.Common.ExternalLoginAlreadyAssociated);
            }

            var result = await UserManager.AddLoginAsync(principal.GetUserId(), login);
            if (!result.Succeeded)
            {
                return ErrorResult(result);
            }

            return Ok(new UserLoginProvidersViewModel
            {
                Provider = externalData.LoginProvider,
                Key = externalData.ProviderKey
            });
        }
        ...
        public async Task<IHttpActionResult> GetExternalLogin([IPrincipal] IPrincipal principal, string provider, string error = null, string redirect_uri = null, bool add = false)
        {
            if (error != null)
            {
            ...
            var externalLogin = ExternalLoginData.FromIdentity(principal.Identity as ClaimsIdentity);
            ...
            var login = new UserLoginInfo(externalLogin.LoginProvider, externalLogin.ProviderKey);
            var user = await UserManager.FindAsync(login);
            var hasRegistered = user != null;
            if (hasRegistered && add)
            {
                Authentication.SignOut(DefaultAuthenticationTypes.ExternalCookie);
                
                return Redirect(redirect_uri + "#error=" + Uri.EscapeDataString(Resources.Common.ExternalLoginAlreadyAssociated));
            }
            ...
            {
                //add -> here we can not associate login with user because we do not have ... user to who 
        ...
        public async Task<IEnumerable<ExternalLoginViewModel>> GetExternalLogins(string returnUrl = null, bool generateState = false, string client_id = null)
        {
            ...
            string state;
            if (!await RefreshTokenManager.ContainsClientAppByIdAsync(client_id))
            {
