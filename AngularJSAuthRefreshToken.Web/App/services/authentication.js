(function (window, document, angular) {
    ...
    angular.module('ea.services')
        .factory('ea.authentication', ['$rootScope', '$timeout', '$q', '$log', 'bk.utils', 'ea.config', 'ea.urlFactory', 'ea.data', 'ea.resources',
            function ($rootScope, $timeout, $q, $log, utils, config, urlFactory, data, messages) {
                var currentExternalLogin = null,
                    currentExternalLoginAdd = false,
                ...
                window.externalLogin = function (token) {
                    ...
                    if (token && -1 < token.indexOf('access_token=')) {
                        ...
                        if (token.provider === 'Local' && !currentExternalLoginAdd) {
                            assignToken(token)
                        }
                        else {
                ...
                var service = {
                    ...
                    externalLogin: function (external, add) {
                        currentExternalLogin = external;
                        currentExternalLoginAdd = add === undefined ? false : add;

                        var url = currentExternalLogin.url;
                        if (currentExternalLoginAdd) {
                            url += "&add=true"
                        }

                        var win = window.open(url, '__externalLogin', 'width=1024,height=620,status=0,personalbar=0,toolbar=0,menubar=0');
                        if (win && win.focus) {
                            win.focus();
                        }
                    },
                    registerExternal: function (params, onSuccessLogin) {
                        if (onSuccessLogin === undefined) {
                            onSuccessLogin = true;
                        }

                        return data.account.registerExternal(params).then(function (emailConfirmation) {
