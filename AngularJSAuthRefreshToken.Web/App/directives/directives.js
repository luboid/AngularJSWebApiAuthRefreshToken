...
(function (angular, document) {
    ...
    function focusElement(e) {
        if (e && e.length) {
            ...
            var nn = e.prop('nodeName');
            if ('INPUT' === nn || 'TEXTAREA' === nn) {
                e[0].select();
            }
        }
    }
    ...
    //#region bkErrorTitle
    var bkErrorTitle = ['$parse', function ($parse) {
        return {
            ...
            link: function ($scope, $element, $attrs, ctrls) {
                var ctrl = ctrls[0],
                    ...
                    messages = ($attrs.bkErrorTitle && $parse($attrs.bkErrorTitle)($scope)) || {};
...
<input type="text" class="form-control" placeholder="Филтър..." ng-model="value" ng-disabled="searchDisabled" />\
<select type="text" class="form-control hidden" ng-model="value" ng-options="obj.value as obj.text for obj in options" ng-disabled="searchDisabled"></select>\
<div class="input-group-btn">\
                            ...
                            case 'b':
                                for (var i in newValue) {
                                    $scope.valid = (
                                        ...
                                        newValue[i] === 'false' ||
                                        newValue[i] === '0' ||
                                        newValue[i] === '1') && $scope.valid;
                                }
                                $scope.valid = newValue.length !== 1 && $scope.valid;
                                break;
                            case 'n':
                                for (var i in newValue) {
...
<th ng-repeat="column in bkFoot" ng-attr-colspan="{{column.colSpan}}" ng-attr-title="{{column.help}}" ng-class="utils.cssClass(column)" ng-bind-html="utils.formatFoot(this, column)"></th>\
    ...
    angular.module('bk.directives', ['bk.dateParser'])
        ...
        .directive('bkClickDirectFocus', [function () {
            return function (scope, element, attrs) {
                function click(event) {
                    scope.$apply(function () {
        ...
        .directive('bkClickDirect', ['$timeout', function ($timeout) {
            return function (scope, element, attrs) {
                function click(event) {
                    //when open popup this need to be done into click handler
                    //because popup blocker catch up
                    scope.$eval(attrs.bkClickDirect);
                }

                if (attrs.bkClickDirect) {
                    element.on('click', click);
                    scope.$on('$destroy', function () {
                        element.off('click', click);
                    });
